            ...
            try
            {
                if(ac.screenCheck(inf.usr,11,2,2,0))
                {
                    ...
                    if (header != null)
                    {
                        ...
                        var lines1 = (from a in db1.QcTraTestsDet.Where(a => a.Lottype == "MAT" && a.TransactionId == inf.recordId && a.RejectedQty > 0 && a.BranchId == inf.usr.bCode && a.CustomerCode == inf.usr.cCode)
                        ...
                        string gin = header.Dat.Value.Year.ToString().Substring(2, 2) + gg.zeroMake(header.Dat.Value.Month, 2) + gg.zeroMake(header.Dat.Value.Day, 2);
                        int? x = findGin(inf.usr, gin);
                        ...
                        foreach (var purline in purlines)
                        {
                            purline.Gin = gin + gg.zeroMake((int)x, 5);
                            x++;
                        }

                        db.InvMaterialManagement.AddRange(purlines);
