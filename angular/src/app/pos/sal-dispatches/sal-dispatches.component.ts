...
@Component({
  selector: 'app-sal-dispatches',
  templateUrl: './sal-dispatches.component.html',
  styleUrls: ['./sal-dispatches.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SalDispatchesComponent implements OnInit {
...
public listdat:Date=new Date();
  ...
  constructor(private acc:AccAccountsService,private adm:AdminGeneralInfoService,private ino:CrmOrdersService,
  ...
  openOld(obj:any)
  {
   ...
   if(obj.pos <= 2)
   {
    ...
    this.spinner.show(undefined,
      ...
      this.ino.getSaleOrder(this.recordID).subscribe(res =>
        {
            var dets:any=res;
...
public delete()
{
  ...
  this.ino.setSaleOrder(header,lines,term, taxe,3).subscribe(res =>
      {
        var result:any=res;
        if(result.result=='OK')
        {
...
public save()
{
  ...
  Swal.fire({  
    title: this.recordID==0? 'Confirms Sale Order Details':'Modifies Sale Order Details',  
    text: 'You will not be able to recover this file!',  
    icon: 'warning',  
    showCancelButton: true,  
    confirmButtonText: 'Yes, confirm it!',  
  ...
  this.ino.setSaleOrder(header,lines, term, taxe,this.recordID==0?1:2).subscribe(res =>
      {
        var result:any=res;
        if(result.result=='OK')
        {
...
this.ino.getCRMSaleOrderRequirements().subscribe(res =>
{
  var det:any=res;
...
public ShowOrders()
{
  ...
  var dat2=dat1;
  this.ino.getSaleOrders(dat1,dat2).subscribe(res =>
    {
