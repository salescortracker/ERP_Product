...
@Component({
  selector: 'app-crm-pre-rep-orders-list',
  templateUrl: './crm-pre-rep-orders-list.component.html',
  styleUrls: ['./crm-pre-rep-orders-list.component.scss']
})
export class CrmPreRepOrdersListComponent implements OnInit {
  public details:any;
  ...
  constructor(private crm:CrmPreSaleReportsService,private adm:AdminGeneralInfoService,
   ...
   listAdd()
   {
     ...
     this.crm.getListofOrders(this.adm.strDate(this.fromdate),this.adm.strDate(this.todate)).subscribe(
       async res => {
         var det:any=res;
         console.log(det);
         if(det)
