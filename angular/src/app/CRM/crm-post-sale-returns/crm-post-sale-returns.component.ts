...
@Component({
  selector: 'app-crm-post-sale-returns',
  templateUrl: './crm-post-sale-returns.component.html',
  styleUrls: ['./crm-post-sale-returns.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CrmPostSaleReturnsComponent implements OnInit {
...
public recordId:number=0;
  ...
  public header:any={
    ...
    constructor(private sal:SalSalesService, private puo:InvPurchaseOrderService,
      private crm:CrmSaleReturnsService,
      ...
      this.crm.getSaleReturnRequirements().subscribe(res =>
        {
          var det:any=res;
          this.seq=det.seq;
    ...
    deleteSaleReturn(obj:any)
    {
      Swal.fire({  
        title: 'Deletes Sale Return Details',  
        ...
        if (result.value) { 
          ...
          this.spinner.show(undefined,
            ...
            this.crm.deleteSaleReturn(obj.recordId).subscribe(res =>
              {
                var det:any=res;
                if(det.result=='OK')
                {
  ...
  this.crm.getSaleReturns(dat1).subscribe(res =>
    {
  ...
  public makeTaxes()
  {
   ...
   private getDismissReason(reason: any): string {
    ...
    this.crm.getSaleReturnSalesList(dat1,dat2).subscribe(res =>
      {
          this.purchases=res;
           console.log(res);
           this.spinner.hide();
  ...
  public getPurchase(obj:any)
  {
    this.spinner.show(undefined,
      ...
      this.crm.getSaleReturnSaleInfo(obj.recordId).subscribe(res =>
        {
  ...
  private valChk():boolean
  {
    ...
    if(x==0)
    {
     ...
     if(x > 0)
     {
      this.adm.showMessage('Retuned qty can not be more than sold','Warning',3);
      return false;
     }
    return true;
  }
  ...
  savePurchase()
  {
    if(this.valChk())
    {
      ...
      Swal.fire({  
        title: 'Confirms Sale Return Details',  
        text: 'You will not be able to recover this file!',  
        icon: 'warning',  
        showCancelButton: true,  
        confirmButtonText: 'Yes, confirm it!',  
        ...
        if (result.value) { 
          ...
          this.spinner.show(undefined,
            ...
            this.crm.setSaleReturn(this.header,lines,taxes,1).subscribe(res =>
              {
                var det:any=res;
                this.spinner.hide();
                if(det.result=='OK')
