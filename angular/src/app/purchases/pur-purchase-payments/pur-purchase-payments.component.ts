...
@Component({
  selector: 'app-pur-purchase-payments',
  templateUrl: './pur-purchase-payments.component.html',
  styleUrls: ['./pur-purchase-payments.component.scss']
})
export class PurPurchasePaymentsComponent implements OnInit {
  ...
  public revereseAccount:number=0;
    constructor(private adm:AdminGeneralInfoService, private pur:PurchasesService,
  ...
  public listAdd()
  {
    this.spinner.show(undefined,
      ...
      this.pur.getPayments(this.adm.strDate(this.listdate)).subscribe(res =>
        {
            this.receipts=res;
            this.spinner.hide();
        });
    ...
    Swal.fire({  
      ...
      if (result.value) { 
        this.spinner.show(undefined,
          ...
          this.pur.deletePayment(obj.recordId).subscribe(res =>
            {
                var det:any=res;
                if(det.result=='OK')
                {
  ...
  public save()
  {
    ...
    {
      Swal.fire({  
        title:  'Confirms Payment Details' ,  
        text: 'You will not be able to recover this file!',  
        icon: 'warning',  
        showCancelButton: true,  
        confirmButtonText: 'Yes, confirm it!',  
      ...
      for(var i=0;i<this.details.length;i++)
      {
       ...
       this.pur.setPayment(header,lines).subscribe(res =>
        {
            var det:any=res;
            if(det.result=='OK')
            {
  ...
  showDet()
  {
    ...
    this.par.GetSupplierCompletePendingBalances(+this.customerid).subscribe(res =>
      {
          var det:any=res;
           this.details=[];
          for(var i=0;i<det.length;i++)
