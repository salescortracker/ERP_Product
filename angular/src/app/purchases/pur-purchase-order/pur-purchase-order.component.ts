...
@Component({
  selector: 'app-pur-purchase-order',
  templateUrl: './pur-purchase-order.component.html',
  styleUrls: ['./pur-purchase-order.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PurPurchaseOrderComponent implements OnInit {
      ...
      this.ord.GetInvPurchaseOrder(this.recordID).subscribe(res =>
        {
              ...
              if(det.result=='OK')
            {
              ...
              this.cmbsupplier=header.vendorid;
              if(sup.length > 0)
              {
                this.supdetail={
               ...
               // this.refQuo=header.refQuotation;
                ...
                for(var i=0;i<det.lines.length;i++)
                {
                  this.itemdetails.push({
                    ...
                    requestid: +det.lines[i].purRequest,
                    request: det.lines[i].branchId,
                   umid:+det.lines[i].um,
                    um:det.lines[i].itemDescription,
                    qty:+det.lines[i].qty,
                    rat:+det.lines[i].rat,
                    value:+det.lines[i].qty*+det.lines[i].rat,
                    reqdby:new Date(det.lines[i].reqdBy)
                  });
                }
...
public save()
{
  ...
  if(this.supdetail.availablecredit < this.supdetail.cbalance && this.supdetail.restrictmode==3)
  {
    this.adm.showMessage('Supplier Credit limit is over please contact finance department','Warning',3);
    return;
  }

  var title:string=this.recordID==0? 'Confirms Purchase Order Details':'Modifies Purchase Order Details';
  if(this.supdetail.availablecredit < this.supdetail.cbalance && this.supdetail.restrictmode==2)
  {
    title='Supplier Credit limit is over Do you want to proceed';
  }
...
public findAddresses(e)
{
  ...
  if(det)
  {
    this.supdetail={
      ...
      availablecredit:det[0].availableCredit,
  cbalance:det[0].balanceAmt,
  restrictmode:det[0].restrictMode,
