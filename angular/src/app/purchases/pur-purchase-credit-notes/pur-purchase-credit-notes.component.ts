...
@Component({
  selector: 'app-pur-purchase-credit-notes',
  templateUrl: './pur-purchase-credit-notes.component.html',
  styleUrls: ['./pur-purchase-credit-notes.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PurPurchaseCreditNotesComponent implements OnInit {
  public materials:any;
  public title:string='';
  now:Date= new Date();
  public listdat:any={year: this.now.getFullYear(), month: this.now.getMonth() + 1, day: this.now.getDate()};
public purchasecrnotes:any;
  public mir:string='';
...
public recordId:number=0;
  ...
  public header:any={
    ...
    deleteCreditNote(obj:any)
    {
      Swal.fire({  
        title: 'Deletes Purchase Credit Note Details',  
        ...
        if (result.value) { 
          ...
          this.spinner.show(undefined,
            ...
            this.pur.deletePurchaseCreditNote(obj.recordId).subscribe(res =>
              {
                var det:any=res;
                if(det.result=='OK')
                {
  ...
  this.pur.getPurchaseCreditNotes(dat1).subscribe(res =>
    {
        this.purchasecrnotes=res;
         console.log(res);
         this.spinner.hide();
    });
  ...
  public getPurchase(obj:any)
  {
    this.spinner.show(undefined,
      ...
      this.pur.getPurchaseReturnPurchaseInfo(obj.recordId).subscribe(res =>
        {
          this.note.TransactionId=obj.recordId;
  ...
  private valChk():boolean
  {
     ...
     if(+this.note.Amt <= 0)
     {
       this.adm.showMessage('Credit note amount not mentioned','Warning',3);
       return false;
     }
     if(this.note.Descriptio.trim() =='')
     {
       this.adm.showMessage('Description not mentioned','Warning',3);
       return false;
     }
  ...
  savePurchase()
  {
    if(this.valChk())
    {
      ...
      Swal.fire({  
        title: 'Confirms Credit Note Details',  
        text: 'You will not be able to recover this file!',  
        icon: 'warning',  
        showCancelButton: true,  
        confirmButtonText: 'Yes, confirm it!',  
        ...
        if (result.value) { 
          ...
          this.spinner.show(undefined,
           ...
           this.note.Amt= +this.note.Amt;
            
            this.pur.setPurchaseCreditNote(this.note).subscribe(res =>
              {
                var det:any=res;
                this.spinner.hide();
